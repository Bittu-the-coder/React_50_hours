...
// Arrays for generating variations
const companies = ["Nike", "Adidas", "Puma", "Vans", "Reebok", "New Balance"];
const colors = ["black", "white", "red", "blue", "green", "yellow", "pink", "gray"];
const categories = ["sneakers", "flats", "sandals", "heels", "boots", "running"];
const priceRanges = [
  { prev: "140,00", new: "50" },
  { prev: "180,00", new: "100" },
  { prev: "200,00", new: "150" },
...
const generateMoreItems = (count) => {
  const additionalItems = [];
  const baseImages = baseData.map((item) => item.img);

  for (let i = 0; i < count; i++) {
    const randomBaseImage = getRandomElement(baseImages);
    const randomPrice = getRandomElement(priceRanges);

    additionalItems.push({
      img: randomBaseImage,
      title: `${getRandomElement(companies)} ${getRandomElement(categories)} ${i + 1}`,
      star: '<AiFillStar className="rating-star" />',
      reviews: `(${Math.floor(Math.random() * 500)} reviews)`,
      prevPrice: `$${randomPrice.prev}`,
      newPrice: randomPrice.new,
...
// Combine original data with additional items
const data = [...baseData, ...generateMoreItems(200)];
